...
import { getDocumentsByPlanning } from '@/api/document';
import { downloadAndStoreDocument } from '@/utils/download';
import DocumentModal from '@/components/modal/DocumentModal';
import { DocumentItem } from '@/components/modal/DocumentModal';
...
const AccomodationScreen = () => {
  ...
  const [docModalItems, setDocModalItems] = useState<DocumentItem[]>([]);
  ...
  const loadPlannings = async () => {
    ...
    const filtered = all.filter((planning) => {
      if (planning.type !== 'hebergement') return false;
  ...
  const handleAttachment = async (planning: PlanningData) => {
    try {
      const documents = await getDocumentsByPlanning(planning.id);

      if (documents.length === 0) {
        Alert.alert('Aucun document', 'Aucun document associé à ce planning.');
        return;
      }

      setDocModalItems(documents);
      setDocModalVisible(true);
    } catch (error) {
      Alert.alert('Erreur', 'Impossible de récupérer les documents.');
    }
  };
      ...
      {date && (
        <View style={{ paddingHorizontal: METRICS.padding, paddingTop: 12, paddingBottom: 6 }}>
          <Text style={{ fontSize: 18, fontWeight: 'bold' }}>
            Hébergements du {formatToFrenchDate(date as string)}
          </Text>
        </View>
      )}
      ...
      <View style={{ flex: 1 }}>
        {accomodation.length === 0 ? (
          <View style={{ padding: METRICS.padding }}>
            <Text>Aucun hébergement ce jour-là</Text>
          </View>
        ) : (
          <View style={{ flex: 4 }}>
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <PlanningCard
                    ...
                    onConfirmBooking={() => handleConfirmBooking(item)}
                    onDelete={() => deletePlanningWithConfirm(item, refresh)}
                    onAttachmentPress={() => handleAttachment(item)}
                  />
                </View>
              )}
            />
        ...
        <View style={{ flex: 1, paddingHorizontal: METRICS.padding, justifyContent: 'center', alignItems: 'center' }}>
          <PrimaryButton title="Ajouter un hébergement" onPress={handleAdd} />
        </View>
      </View>
      {docModalVisible && (
        <DocumentModal
          visible={docModalVisible}
          documents={docModalItems}
          onClose={() => {
